...
fn setup_db(connection: &sqlite::Connection) {
    connection.execute("CREATE TABLE IF NOT EXISTS FEEDS ( TITLE TEXT PRIMARY KEY, URL TEXT UNIQUE);").unwrap();
...
fn import_opml_into_db(opml_path: &str, connection: &sqlite::Connection) {
    ...
    let mut insert_stmt = connection.prepare("INSERT OR REPLACE INTO FEEDS (TITLE, URL) VALUES (?, ?);").unwrap();
...
fn import_feed_into_db(title: &str, url: &str, connection: &sqlite::Connection) {
    ...
    for entry in &feed.unwrap().entries {
        ...
        insert_stmt.bind(2, text_content_or(&entry.title, "(no title)")).unwrap();
...
fn refresh_feeds(connection: &sqlite::Connection) {
    ...
    while let sqlite::State::Row = select_stmt.next().unwrap() {
        items.push((select_stmt.read::<String>(0).unwrap(), select_stmt.read::<String>(1).unwrap()));
    }
...
fn main() {
    ...
    if 1 < 2 {
        import_opml_into_db(opml_path, &connection);
